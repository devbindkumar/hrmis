...
async def ensure_indexes():
    ...
    # to coexist. Guard against a legacy non-partial index left over from a
    # previous deployment: drop-and-recreate is cheap for this tiny collection.
    try:
        ...
        try:
            await db.extensions.drop_index("company_id_1_employee_id_1")
        except Exception:
            pass
        await db.extensions.create_index(
            [("company_id", 1), ("employee_id", 1)],
    ...
    await db.attendance_events.create_index([("user_id", 1), ("ts", -1)])
    await db.password_reset_otps.create_index("id", unique=True)
    await db.password_reset_otps.create_index([("user_id", 1), ("created_at", -1)])
    await db.password_reset_otps.create_index([("email", 1), ("used", 1)])
    await db.meeting_rooms.create_index("id", unique=True)
    await db.meeting_rooms.create_index([("company_id", 1), ("active", 1)])
    await db.meetings.create_index([("company_id", 1), ("room_id", 1), ("starts_at", 1)])
    await db.meetings.create_index([("company_id", 1), ("approval_status", 1)])
