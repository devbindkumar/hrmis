...
async def admin_overview(admin: dict = Depends(require_roles("super_admin", "hr", "manager"))):
    ...
    # Today attendance
    attendance = await db.attendance.find({"date": today}, {"_id": 0}).to_list(500)
    ...
    # WFH today
    wfh_docs = await db.wfh_requests.find({"status": "approved", "date": today}, {"_id": 0}).to_list(500)
    wfh = len(wfh_docs)
    ...
    for i in range(6, -1, -1):
        ...
        att_day = await db.attendance.find({"date": day}, {"_id": 0}).to_list(500)
        leaves_day = await db.leave_requests.count_documents({
            "status": "approved",
            "start_date": {"$lte": day},
            "end_date": {"$gte": day},
        })
        wfh_day = await db.wfh_requests.count_documents({"status": "approved", "date": day})
    ...
    pending_leaves = await db.leave_requests.find({"status": "pending"}, {"_id": 0}).sort("created_at", -1).to_list(8)
    pending_wfhs = await db.wfh_requests.find({"status": "pending"}, {"_id": 0}).sort("created_at", -1).to_list(8)

    # latest announcement
    last_announcement = await db.announcements.find_one({}, {"_id": 0}, sort=[("created_at", -1)])
...
async def employee_overview(user: dict = Depends(get_current_user)):
    ...
    balances = await db.leave_balances.find({"user_id": user["id"]}, {"_id": 0}).to_list(50)
    ...
    announcements = await db.announcements.find({}, {"_id": 0}).sort("created_at", -1).limit(3).to_list(3)
